...
like $cmd->{test}, qr|system.+cat.+Makefile\.PL|, 'find before run coderef';
unless (DMAKE) {
    like $cmd->{test}, qr|sub { print scalar localtime }->\(\); |, 'find before run code';
    like $cmd->{test}, qr|\\\$\$ENV{__TEST__} = 1|, 'find escaped sigil';
}
else {
    like $cmd->{test}, qr|sub {{ print scalar localtime }}->\(\); |, 'find before run code';
    like $cmd->{test}, qr|\$\$ENV{{__TEST__}} = 1|, 'find escaped sigil';
